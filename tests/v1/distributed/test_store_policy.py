...
class TestStripedStorePolicyAdapterIndex:
    ...
    def test_cross_process_stable_md5(self):
        """MD5 is deterministic across processes (no PYTHONHASHSEED).

        We verify by recomputing the MD5 independently and comparing.
        """
        import hashlib
        ...
        expected = int.from_bytes(
            hashlib.md5(str(key).encode()).digest()[:8], "big"
        ) % num
        assert idx == expected
